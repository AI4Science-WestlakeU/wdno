...
class Attention(nn.Module):
    ...
    def forward(
        ...
        q, k, v = rearrange_many(qkv, '... n (h d) -> ... h n d', h = self.heads) # h is heads, d is dim_head, n is dim
        # scale
        del qkv
...
class Unet3D(nn.Module):
    def __init__(
        ...
        self.feature_extractor = nn.MaxPool2d(16, stride=16) # input 64, (1,2,4)
        ...
        self.out_dim = default(out_dim, channels // 2) # !!!!!!!!!!! in prediction theta, use channels  // 2
        self.final_conv = nn.Sequential(
            block_klass(dim * 2, dim),
            nn.Conv3d(dim, self.out_dim, 1)
        )
        
        self.thetas_predictor = nn.Sequential(
            # nn.Linear(512, 128),# (1,2,4,8)
            nn.Linear(256, 128), # (1,2,4)  
            nn.ELU(),
            nn.Linear(128, 32),
            nn.ELU(),
            nn.Linear(32, 1),
    ...
    def forward(
        ...
        b,t,h,w = x.shape
        state = x[:,:60,:,:].reshape(b,20,3,h,w)
        bd = x[:,60:,:,:].reshape(b,20,3,h,w)
        x = torch.cat([state,bd],dim=2)
        ...
        feature = self.feature_extractor(x.reshape(b,  x.shape[1] * x.shape[2], x.shape[-2], x.shape[-1])).reshape(b, x.shape[1], x.shape[2]) # feature: [batch, 512, 20] for (1,2,4,8)
        ...
        # b,c,t,h,w = x.shape
        # x = x.permute(0,2,1,3,4)
        # state = x[:,:,:3,:,:].reshape(b,60,h,w)
        # bd = x[:,:,3:,:,:].reshape(b,60,h,w)
        
        b,c,t,h,w = x.shape # [batch, 3, 20, 128, 128]
        x = x.permute(0,2,1,3,4)
        state = x.reshape(b,60,h,w) # [batch, 60, 128, 128]
        
        # print("::::",x.shape)
        # print(b,120,h,w)
        #x = x.reshape(b,120,h,w)
        
        # return torch.cat([state,bd],dim=1)
    
        thetas = self.thetas_predictor(feature.permute(0, 2, 1)).squeeze(-1) # theta: [batch, 20]
