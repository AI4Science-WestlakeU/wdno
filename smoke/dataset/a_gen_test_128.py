...
def plot_vector_field_128(velocity):
    ...
    # plt.savefig(f'vector_field/{frame_}.png', dpi=50)
    # plt.show()
...
def get_envolve(sim,pre_velocity,frame,control_write,vx=None,vy=None):
    if(vx==None and vy==None):
        ...
        if frame % record_scale == 0:
            control_write[:,:,0,int(frame/record_scale)] = divergent_velocity[0,:,:,0]
            control_write[:,:,1,int(frame/record_scale)] = divergent_velocity[0,:,:,1]

        # Uncontroled Velocity <- Velocity from last step
        ...
        # Concate to make current_vel_field
        # current_vel_field - numpy array - shape[1,128,128,2]
        ...
        # StaggeredGried: numpy array - StaggeredGrid
        Current_vel_field = StaggeredGrid(current_vel_field)
...
def get_intial_state(sim,xs,ys,vxs,vys,density_write,density_set_zero_write,velocity_write,control_write):
    ...
    # save initial velocity field
    initial_vel_path = './intial_vel.npy'
    np.save(initial_vel_path, optimizable_velocity[0,:,:,:])
    ...
    density_write[:-1,:-1,:,0] = advected_density[0,:,:,:]
    density_set_zero_write[:-1,:-1,:,0] = advected_density[0,:,:,:]
    
    velocity_write[:,:,0,0] = loop_velocity.staggered[0,:,:,0]
    velocity_write[:,:,1,0] = loop_velocity.staggered[0,:,:,0]
...
def write_vel_density(loop_velocity,loop_advected_density,loop_density_no_set,density_write,density_set_zero_write,velocity_write,frame,smoke_outs_128_record,smoke_outs_128):
    ...
    density_write[:-1,:-1,:,int(frame/record_scale)] = loop_density_no_set[0,:,:,:] 
    density_set_zero_write[:-1,:-1,:,int(frame/record_scale)] = loop_advected_density[0,:,:,:] 
    velocity_write[:,:,0,int(frame/record_scale)] = loop_velocity.staggered[0,:,:,0]
    velocity_write[:,:,1,int(frame/record_scale)] = loop_velocity.staggered[0,:,:,1]
    ...
    smoke_outs_128[int(frame/record_scale),-1] = np.sum(loop_density_no_set[0,:,:,:])
...
def exp2_same_side_128(is_train_, fix_velocity_, Test_, branch_num, data_savepath):
    ...
    for scene_index in range(begin_sim_set, scenecount+begin_sim_set):
        ...
        density_write = np.zeros((128,128,1,record_frame_len), dtype=float)
        density_set_zero_write = np.zeros((128,128,1,record_frame_len), dtype=float)
        velocity_write = np.zeros((128,128,2,record_frame_len), dtype=float)
        control_write = np.zeros((128,128,2,record_frame_len), dtype=float)
...
if __name__ == "__main__":
    exp2_same_side_128(is_train_=False, fix_velocity_=False,Test_=True, branch_num=0, data_savepath='test0507_32_128_128')

# if __name__ == "__main__":
#     parser = argparse.ArgumentParser()
#     parser.add_argument("--test_or_train", type=str, help="(test:input test or train)")
#     # parser.add_argument("--branch_list", type=str, help="branch No")
...
#     # branch_list_str = args.branch_list
