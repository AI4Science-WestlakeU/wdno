...
def plot_velocity_boundary_effect2(velocity):
    ...
    #hor_velocity_array[...,0] = numpy.flipud(velocity.staggered[0,:,:,0])
    hor_velocity_array = velocity.staggered[0,:,:,0]

    ver_velocity_array = np.empty([128, 128], np.float32)
    #ver_velocity_array[...,0] = numpy.flipud(velocity.staggered[0,:,:,1])
...
def plot_loop(loop_advected_density, loop_velocity, target_des_array, frame=None):
    ...
    """
    # ax.scatter(np.where(target_des_array==1)[1], np.where(target_des_array==1)[0], color="white", marker=",")
    # ax.scatter(hor_bound, ver_bound, s=4, color="grey", marker=",")
...
def get_envolve(sim,pre_velocity,frame,control_write,vx=None,vy=None):
    if(vx==None and vy==None):
        ...
        if frame % record_scale == 0:
            control_write[:,:,0,frame] = divergent_velocity[0,::2,::2,0]
            control_write[:,:,1,frame] = divergent_velocity[0,::2,::2,1]
        ...
        # Use phiflow function to generated physic-suitable velocity field
        # Input: Current_vel_field.shape = [1,128,128,2] Type:StaggeredGrid
        # Output: velocity.shape = [1,128,128,2] Type:StaggeredGrid
        # [1,128,128,2] - [1,dim,dim,[vx,vy]]
        ...
        # divergent_velocity.shape = [1,128,128,2]
...
def write_vel_density(loop_velocity,loop_advected_density,loop_density_no_set,density_write,density_set_zero_write,velocity_write,frame,smoke_outs_128_record,smoke_outs_128):
    ...
    density_write[:,:,:,frame] = loop_density_no_set[0,::2,::2,:] 
    density_set_zero_write[:,:,:,frame] = loop_advected_density[0,::2,::2,:] 
    velocity_write[:,:,0,frame] = loop_velocity.staggered[0,::2,::2,0]
    velocity_write[:,:,1,frame] = loop_velocity.staggered[0,::2,::2,1]
    ...
    smoke_outs_128[frame,:-1] = smoke_outs_128_record
    smoke_outs_128[frame,-1] = np.sum(loop_density_no_set[0,::2,::2,:])
...
def loop_write_0423(sim,loop_advected_density,loop_velocity,smoke_outs_128,save_sim_path,vxs,vys,intervals,xs,ys,density_write, \
    ...
    for frame in range(1, intervals[0]):
        ...
        if frame % record_scale == 0:
            ...
            velocity_write[:,:,1,frame] = loop_velocity.staggered[0,::2,::2,1]
            smoke_outs_128[frame,:-1] = smoke_outs_128_record
            smoke_outs_128[frame,-1] = np.sum(loop_advected_density[0,::2,::2,:])
...
#     exp2_same_side_128(is_train_=False, fix_velocity_=False,Test_=True, branch_num=0, data_savepath='test0506_256_64_64')
